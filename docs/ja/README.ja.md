...
| **L4 Research** | 全ユーザー | 要並列計算等 | 研究レベルの応用（R01〜R10、各 30〜90 分） | 🔜 v1.3.0 予定 |

*L1〜L3 は MATLAB Online Basic（無料枠）で完全動作します。*

### Layer 1: Foundation（Base MATLAB のみ）

| # | タイトル | 必要ツールボックス | 学習内容 | Desktop | Online |
|---|---|---|---|:---:|:---:|
...
| # | タイトル | 必要ツールボックス | ドメイン | Desktop | Online |
|---|---|---|---|:---:|:---:|
...
| # | タイトル | 必要ツールボックス | 内容 | Desktop | Online |
|---|---|---|---|:---:|:---:|
| A01 | PCA による化学空間マッピング | 統計・機械学習 | 次元削減・化学空間可視化 | ✔ | ✔ |
| A02 | 分子クラスタリング | 統計・機械学習 | 階層クラスタリング・構造類似性 | ✔ | ✔ |
| A03 | QSAR 回帰 | 統計・機械学習 | LogP 予測・回帰モデル評価 | ✔ | ✔ |
| A04 | 薬物分類 | 統計・機械学習 | SVM / ランダムフォレスト・ROC 曲線 | ✔ | ✔ |
| A05 | ニューラルネットワーク物性予測 | Deep Learning | フィードフォワード NN・物性回帰 | ✔ | ✔ |
| A06 | 用量反応カーブフィット | Curve Fitting | Hill 式・EC50 推定 | ✔ | ✔ |
| A07 | Scaffold 分析と R グループ分解 | 統計・機械学習 | 創薬化学分析 | ✔ | ✔ |
| A08 | 質量分析×ケモインフォマティクス | 信号処理 + 統計 | 同位体パターンマッチング・MS アノテーション | ✔ | ✔ |
| A09 | PFAS・環境スクリーニング | 最適化 + 統計（任意） | SMARTS スクリーニング・Pareto 最適化 | ✔ | ✔ |
| A10 | リード最適化 | 最適化（任意） | 多目的最適化・Derringer-Suich 法 | ✔ | ✔ |

### Layer 4: Research

> 🔜 **未リリース。** モジュール詳細は公開時に掲載します。
>
> L4 Research（R01〜R10）は **v1.3.0** で予定。
