...
`Config` パラメータを受け取ることで、`main_emk.m` から一貫した設定を渡し、オプションライブラリも一括インストールできる。
...
**二重呼び出し防御の根拠**:
MATLAB の `pyenv` はセッション中に一度しか `Version`/`ExecutionMode` を変更できない（Python が "Loaded" 状態になると設定変更不可）。`install()` の末尾や `main_emk.m` の Section 0a から繰り返し呼ばれる可能性があるため、冪等性を保証するガードが必要（ADR-001 rev.3）。
